...
namespace Native.Csharp.App.Event
{
    public class Event_GroupMessage : IEvent_GroupMessage
    {
        ...
        private static Dictionary<long, Repeater> RepeaterDict = new Dictionary<long, Repeater>();
        ...
        public void ReceiveGroupMessage(object sender, GroupMessageEventArgs e)
        {
            ...
            try
            {
                if (Repeater.CheckIfIgnore(e.Msg))
                    return;

                if (!RepeaterDict.ContainsKey(e.FromGroup))//如果群无复读记录
                    RepeaterDict[e.FromGroup] = new Repeater(e.Msg);

                else if (!RepeaterDict[e.FromGroup].CDDec())//注：CDDec()同时做了两件事，需要特别注意
                    return;
                else if (RepeaterDict[e.FromGroup].RandomlyDecide(e.Msg))
                {
                    if (!RepeaterDict[e.FromGroup].AlreadyRepeated())
                        RepeaterDict[e.FromGroup].DoRepeat(e);
                }
                else //Roll的结果判定不复读
                    RepeaterDict[e.FromGroup].Update(e.Msg);

                if (RepeaterDict[e.FromGroup].Delaying())
                    RepeaterDict[e.FromGroup].DoDelayRepeat(e);
            }
            catch (Exception ex)
            {
                Common.CqApi.AddLoger(Sdk.Cqp.Enum.LogerLevel.Error, "复读机", ex.Message);
            }
